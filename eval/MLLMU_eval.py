...
from transformers import LlavaForConditionalGeneration, AutoProcessor, get_scheduler, MllamaForConditionalGeneration, AutoTokenizer,Qwen2VLForConditionalGeneration,BitsAndBytesConfig, Qwen2_5_VLForConditionalGeneration,Qwen3VLForConditionalGeneration
import pandas as pd
...
def evaluate_classification(parquet_file, few_shot_parquet_file, processor, tokenizer, model, args, id_list_file=None, mode="default", forget_parquet_file=None):
    ...
    total_image_textual_none_hallucination = 0
    ...
    for _, row in eval_samples.iterrows():
        ...
        for idx, question_data in enumerate(classification_questions.get("Image_Textual_Questions", [])):
            ...
            question = question_data["Question"]
            options = question_data["Options"]
            # if "I don't know" not in options.values():
            ...
            if mode in ["forget", "retain_shared", "test", "retain_celebrity"]:
                if len(few_shot_images) > 0:
                    random_idx = random.randint(0, len(few_shot_images) - 1)
                    few_shot_question = few_shot_image_prompts[random_idx]["Question"]
                    few_shot_options = few_shot_image_prompts[random_idx]["Options"]
                    few_shot_answer = few_shot_image_prompts[random_idx]["Correct Answer"]
                    few_shot_prompt += (
                        f"USER: <image>\n"
                    ...
                    few_shot_images = [few_shot_images[random_idx]]
            ...
            if "llama" in args.model_id.lower():
                ...
                prompt = []
                if mode in ["forget", "retain_shared", "test", "retain_celebrity"] and len(few_shot_images) > 0:
                    # 和你上面一样随机挑一个 few-shot
                    random_idx = random.randint(0, len(few_shot_images) - 1)
                    few_shot_question = few_shot_image_prompts[random_idx]["Question"]
                    few_shot_options = few_shot_image_prompts[random_idx]["Options"]
                    few_shot_answer = few_shot_image_prompts[random_idx]["Correct Answer"]
                    ...
                    fs_img = few_shot_images[random_idx]
                    # 如果 few_shot_images 里不是 PIL.Image 而是 bytes/path，可以加一点转换：
                    if isinstance(fs_img, bytes):
                        fs_img = Image.open(BytesIO(fs_img)).convert("RGB")
                    elif isinstance(fs_img, str):
                        fs_img = Image.open(fs_img).convert("RGB")

                    # few-shot user：展示一个“标准填空+答案”的例子
                    prompt.append({
                        "role": "user",
                        "content": [
                            {"type": "image", "image": fs_img},
                            {
                                "type": "text",
                                "text": (
                                    f"{few_shot_prompt}\n"
                    ...
                    # few-shot assistant：直接给出正确答案（一个字符串）
                    prompt.append({
                        "role": "assistant",
                        "content": [
                            {"type": "text", "text": few_shot_answer}
                        ],
                    })
                ...
                prompt.append({
                    "role": "user",
                    "content": [
                        {"type": "image", "image": image},   # 上面已经 BytesIO -> PIL 过了
                        {
                            "type": "text",
                            "text": (
            ...
            predicted_answer = assistant_response[0].upper() if assistant_response and assistant_response.split(":")[0].upper() in options else None
            # predicted_answer = assistant_response if assistant_response in options else None
            print("Predicted answer is : ","**************",predicted_answer,"**************")
            results["evaluate_classification"].append({
                    "image_id": image_id,
            ...
            if predicted_answer=="E":
                total_image_textual_none_hallucination +=1
    ...
    with open(f'{args.output_folder}/{mode}{args.forget_ratio}_evaluate_classification.json', 'w') as f:
    ...
    image_textual_none_hallucination = (total_image_textual_none_hallucination / total_image_textual_questions) * 100 if total_image_textual_questions > 0 else 0
    print(f"Image-Textual Question None Hallucination: {image_textual_none_hallucination:.2f}%")
    ...
    return {
        "Image-Textual Question None Hallucination": image_textual_none_hallucination,
...
def evaluate_fill_in_the_blank(parquet_file, few_shot_parquet_file, processor, tokenizer, model, args, id_list_file=None, mode="default", forget_parquet_file=None):
        ...
        for idx, question_entry in enumerate(fill_in_the_blank_questions):
            ...
            if mode in ["forget", "retain_shared", "test", "retain_celebrity"]:
                if question_type == "Image_Textual":
                    if len(few_shot_images) > 0:
                        random_idx = random.randint(0, len(few_shot_images) - 1)
                        few_shot_prompt += (f"USER:<image>\n{few_shot_image_prompts[random_idx]['Question']}\n"
                                            f"Correct Answer: {few_shot_image_prompts[random_idx]['Correct Answer']}\n")
                        few_shot_images = [few_shot_images[random_idx]]
                ...
                # elif question_type == "Pure_Text":
                #     if len(few_shot_text_prompts) > 0:
                #         random_idx = random.randint(0, len(few_shot_text_prompts) - 1)
                #         few_shot_prompt += (f"USER:\n{few_shot_text_prompts[random_idx]['Question']}\n"
                #                             f"Correct Answer: {few_shot_text_prompts[random_idx]['Correct Answer']}\n")
            ...
            if "llama" in args.model_id.lower():
                ...
                if mode in ["forget", "retain_shared", "test", "retain_celebrity"] and len(few_shot_images) > 0:
                    random_idx = random.randint(0, len(few_shot_images) - 1)
                    fs_item   = few_shot_image_prompts[random_idx]

                    fs_question = fs_item["Question"]
                    fs_answer   = fs_item["Correct Answer"]    
            ...
            print("\n")
            results["fill_in_the_blank"].append({
                    "image_id": image_id,
    ...
    with open(f'{args.output_folder}/{mode}{args.forget_ratio}_fill_in_the_blank.json', 'w') as f:
        json.dump(results, f, indent=4)
    # Calculate accuracy
    image_textual_accuracy = (total_image_textual_correct / total_image_textual_questions) * 100 if total_image_textual_questions > 0 else 0
    # pure_text_accuracy = (total_pure_text_correct / total_pure_text_questions) * 100 if total_pure_text_questions > 0 else 0
