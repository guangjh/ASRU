...
# from transformers import LlavaForConditionalGeneration,AdamW, AutoProcessor, get_scheduler, MllamaForConditionalGeneration, AutoTokenizer,Qwen2VLForConditionalGeneration,BitsAndBytesConfig, Qwen2_5_VLForConditionalGeneration
...
def load_model_and_processor(args):
    ...
    if "llava" in args.model_id:
        ...
        model.print_trainable_parameters()
        processor = AutoProcessor.from_pretrained(args.model_id)
        processor.tokenizer.padding_side = "left"  # Ensure right padding
    else:
...
def main(args):
    ...
    for idx, row in forget_df.iterrows():
        forget_id.append(row['ID'])

    retain_id = []
    for idx, row in retain_df.iterrows():
        retain_id.append(row['ID'])
